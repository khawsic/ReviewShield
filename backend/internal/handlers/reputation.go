...
func calculateReputationScore(averageRating float64, totalReviews int, positiveReviews int, respondedReviews int) ReputationResult {
	...
	return ReputationResult{
		Score: total, Grade: grade, RatingScore: ratingScore,
		VolumeScore: volumeScore, SentimentScore: sentimentScore,
		ResponseScore: responseScore, TotalReviews: totalReviews,
		AverageRating: averageRating, PositivePercent: positivePercent,
		ResponseRate: responseRate, Trend: trend, Message: message,
	}
}

func GetReputationScore(c *gin.Context) {
	businessID := c.GetInt("business_id")
	var totalReviews, positiveReviews, respondedReviews int
	...
	`, businessID)
	err := row.Scan(&totalReviews, &averageRating, &positiveReviews, &respondedReviews)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch stats"})
		return
	}
	result := calculateReputationScore(averageRating, totalReviews, positiveReviews, respondedReviews)
